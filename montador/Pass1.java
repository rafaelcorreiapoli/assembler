...
class Pass1 extends Pass {
     ...
     *
     */
    public boolean defineNewOrigin(String arg) {
        ...
        try {
            int newLoc = getDecNumber(arg);
            if (newLoc > LAST_VAL_ADDR || newLoc < 0) {
                return false;
            } else {
    ...
    private boolean endAsm(String arg) {
         ...
         *
         * */
        return (locationCounter < LAST_VAL_ADDR && locationCounter > 0);
    }
    ...
    private boolean reserveBlock(String arg) {
         ...
         * */
        try {
            Integer finalLoc = locationCounter + getDecNumber(arg) * 2;
            return (finalLoc < LAST_VAL_ADDR && finalLoc > 0);
        } catch (AssemblerException e) {
            e.printStackTrace();
        }
        return false;
